...
from src.mgm_floyd_v2 import mgm_floyd
...
test_num_graph = 4  # number of graphs in each test
...
for i_test in range(len(class_name)):
    print("**************************************************************************")
    print("Test offline multi-graph matching on class {}".format(class_name[i_test]))
    time_cost = []
    accuracy = []
    for i_iter in range(test_iter):
        ...
        test_data_path = "1data" + "/" + "TestPrepare" + "/" + class_name[i_test] + "/" + "test_data_" + str(i_iter)
        ...
        tic = time.time()
        init_mat_path = "1data" + "/" + "TestPrepare" + "/" + class_name[i_test] + "/" + "init_mat_" + str(i_iter)
        if os.path.exists(init_mat_path):
            ...
            Kt = torch.tensor(data.K).reshape(-1, n * n, n * n)
