...
def pairwise_score(X, i,j):
    ...
    return (1 - sum_/ (num_graphs*num_nodes))
...
def fast_spfa(K, X, num_graph, pc_ma):
    """
    ...
    if (num_graph != 16):
        pc_ma_ = np.zeros((num_graph,num_graph))
        pc_ma_[:-1, :-1] = pc_ma
        for pre_g in range(num_graph - 1):
            pc_ma_[pre_g][num_graph -1] = pairwise_score(X, pre_g, num_graph -1)
            pc_ma_[num_graph - 1][pre_g] = pairwise_score(X, num_graph -1 , pre_g)
        pc_ma = pc_ma_
    ...
    labels_ = cluster.fit_predict(affi_ma)
    # print(labels_)
